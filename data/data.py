...
# 读取三元组文件
h_r_t_name = [":START_ID", ":END_ID", "role"]
h_r_t = pd.read_table("tuple.txt", sep=' ', names=h_r_t_name)
print(h_r_t.info())
print(h_r_t.head())

flag = {}
file = open("entity_flag_math.txt", "r")
...
for i in entity_t:
    entity.add(i)
# 保存节点文件
csvf_entity = open("entity_math.csv", "w", newline='', encoding='utf-8')
...
for i in range(len(entity)):
    w_entity.writerow(("e" + str(i), entity[i], flag[entity[i]]))
...
h_r_t.pop('role')
h_r_t.to_csv("roles_math.csv", index=False)